...
- **SAM 모델 로딩**: 백그라운드에서 안전한 모델 로딩 및 초기화
- **진행률 표시**: 모델 로딩 진행 상황을 시각적으로 표시
- **워커 스레드**: 논블로킹 UI로 무거운 작업 처리
...
3. **자동 모델 로딩**: 선택한 모델이 백그라운드에서 자동 로딩 (진행률 표시)
4. **매개변수 설정**: 우측 속성 패널에서 SAM 매개변수 조정
5. **세그멘테이션 실행**: `세그멘테이션 실행` 버튼 클릭 *(구현 예정)*
6. **결과 확인**: 뷰포트에서 마스크 오버레이 확인 *(구현 예정)*
7. **벡터 편집**: 필요시 개별 벡터 수정 *(구현 예정)*
8. **DXF 내보내기**: `DXF 내보내기` 버튼으로 저장 *(구현 예정)*
...
│   ├── progress_dialog.py  # 진행률 다이얼로그 (신규)
│   └── toolbar.py          # 도구모음
├── core/                   # 핵심 로직
│   ├── sam_processor.py    # SAM 모델 처리
│   ├── sam_worker.py       # SAM 워커 스레드 (신규)
